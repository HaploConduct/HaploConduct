...
def main():
    ...
    parser.add_argument('--min_clique_size', dest='min_clique_size', type=int, default=4)
    ...
    parser.add_argument('--transitive_edges', dest='transitive_edges', type=int, default=2)
...
#                while overlap_counts[-1] > 0 and read_counts[-1] != read_counts[-2]:
                run_merging_it(args.merging_threshold, 100, args.min_overlap_len, 1, "false", "false")
                pm_count += 1
        elif args.error_correction:
            run_first_it_cliques(args.fastq, args.overlaps, args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, args.min_clique_size, args.fno, "true", no_inclusions)
        elif args.cliques_first_it or args.cliques:
            run_first_it_cliques(args.fastq, args.overlaps, args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, args.min_clique_size, args.fno, "false", no_inclusions)
        else:
    ...
    if (args.pre_merging or args.cntinue) and args.error_correction:
        run_clique_it(args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, args.min_clique_size, args.fno, "true", no_inclusions, 1000)
    elif (args.pre_merging or args.cntinue) and args.cliques_first_it:
        run_clique_it(args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, args.min_clique_size, args.fno, "false", no_inclusions, 1000)
    ...
    while overlap_counts[-1] > 0 and read_counts[-1] != read_counts[-2]:
        if args.merging:
            while overlap_counts[-1] > 0 and read_counts[-1] != read_counts[-2]:
                if args.cliques:
                    run_merging_it(args.edge_threshold, 100, args.min_overlap_len, 3, "false", "false")
                else:
                    run_merging_it(args.merging_threshold, 0, args.min_overlap_len, 3, "false", "false")
            if args.cliques:
                run_clique_it(args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, 2, args.fno, "false", no_inclusions, 0)
        elif args.cliques:
            run_clique_it(args.edge_threshold, args.min_overlap_perc, args.min_overlap_len, args.min_clique_size, args.fno, "false", no_inclusions, 0)
...
def run_first_it_cliques(fastq, overlaps, edge_threshold, min_overlap_perc, min_overlap_len, min_clique_size, FNO, error_correction, no_inclusions):
    global iteration, max_read_lengths, max_coverages_comp_times, read_counts, overlap_counts, outputdir, transitive_edges
    iteration += 1
    print "\n****************************************"
...
def run_merging_it(edge_threshold, min_overlap_perc, min_overlap_len, FNO, optimize, cliques):
    global iteration, max_read_lengths, max_coverages_comp_times, read_counts, overlap_counts, outputdir, transitive_edges
    iteration += 1
    print "\n*******************************"
...
def run_clique_it(edge_threshold, min_overlap_perc, min_overlap_len, min_clique_size, FNO, error_correction, no_inclusions, keep_singletons):
    global iteration, max_read_lengths, max_coverages_comp_times, read_counts, overlap_counts, outputdir, transitive_edges
    iteration += 1
    print "\n*******************************"
